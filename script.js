 ...
 * How: Uses transform-origin and scale animations to make cells expand/contract from their centers
 */
function initFoamAnimation() {
    const foamCells = document.querySelectorAll('.foam-cell');
    ...
    requestAnimationFrame(() => {
        foamCells.forEach((cell, index) => {
            try {
                ...
                // Vary animation duration slightly for more organic movement
                const baseDuration = cell.classList.contains('interstitial') ? 8 : 20;
                const durationVariation = (Math.random() - 0.5) * 4;
                cell.style.animationDuration = `${baseDuration + durationVariation}s`;
            } catch (e) {
